...
class GetBannersData extends AsyncTask<Void, Void, String>

{
    ...
    protected String doInBackground(Void... params) {
        ...
        try {
            Log.e("GetBannersData","2>>" + params);

            String str_response = MyUtility.hitGetBannersAPI(MyApplication.ctx, MyUtility
                    .getbannersDataAPI);

            if (str_response != null && !TextUtils.isEmpty(str_response)) {
                Log.e("GetBannersData",">>" + str_response);
                ...
                ArrayList<BannersModel> newArray = new ArrayList<>();
                for (int i = 0; i < jsonObject_getDat.getJSONArray("content").length(); i++) {
                    ...
                    Log.e("GetBannersData",">>" + jsonObject.optString("title"));
                    ...
                    BannersModel trendingNewsModel = new BannersModel(id, title, category, status, redirectLink, imageUrl);
                    newArray.add(trendingNewsModel);
                }
                Log.e("GetBannersData","newArray>00>" + newArray.size());
...
//                editor.putString(KEY_CUSTOM_MODEL_ARRAY, json);
                ...
                if (save_json_jsonobject != null) {
                    if (!TextUtils.isEmpty(save_json_jsonobject.toString())) {
                        ...
                        MyApplication.app_editor.putString("save_app_banners_newm_Json", String.valueOf
                                (json));
