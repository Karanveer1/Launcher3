...
public class PackageUpdatedTask extends BaseModelUpdateTask {
    ...
    public void execute(@NonNull final LauncherAppState app, @NonNull final BgDataModel dataModel,
        ...
        switch (mOp) {
            case OP_ADD: {
                ...
                if (N == 1 && isCloneAppNotShowing(context, mUser, packages[0])) {
                    flagOp = FlagOp.NO_OP;
                    break;
                }
                //end
    ...
    private boolean isCloneAppNotShowing(Context context, UserHandle user, String packageName) {
        final String[] supportCloneApp = context.getResources()
                .getStringArray(com.android.internal.R.array.cloneable_apps);
        final List<String> supportCloneAppList = Arrays.asList(supportCloneApp);
        boolean isClone = isUserTypeProfile(context, user, UserManager.USER_TYPE_PROFILE_CLONE);
        boolean result = isClone && !supportCloneAppList.contains(packageName) ?
                true : false;
        return result;
    }
    //end
