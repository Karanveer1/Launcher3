...
public class LoaderTask implements Runnable {
    ...
    private final String[] mSupportApps;
    private final List<String> mSupportAppsList;
    ...
    public LoaderTask(@NonNull LauncherAppState app, AllAppsList bgAllAppsList, BgDataModel bgModel,
        ...
        mSupportApps = mApp.getContext().getResources()
                .getStringArray(com.android.internal.R.array.cloneable_apps);
        mSupportAppsList = Arrays.asList(mSupportApps);
        //end
    }
                ...
                case Favorites.ITEM_TYPE_DEEP_SHORTCUT:
                    ...
                    if (c.restoreFlag != 0) {
                        ...
                        if (unlockedUsers.get(c.serialNumber)) {
							...
							LauncherAppMonitor.getInstance(mApp.getContext()).getLauncher().setShortcutEnableStatus(false);
                        }
    ...
    private List<LauncherActivityInfo> loadAllApps() {
        ...
        for (UserHandle user : profiles) {
            ...
            //sprocomm add begin: app clone
            boolean isClone = isUserTypeProfile(mApp.getContext(), user, UserManager.USER_TYPE_PROFILE_CLONE);
            if (isClone) {
                for (int i = apps.size() - 1; i >= 0; i--) {
                    String packageName = apps.get(i).getActivityInfo().packageName;
                    if(!mSupportAppsList.contains(packageName)) {
                        apps.remove(i);
                    }
                }
            }
