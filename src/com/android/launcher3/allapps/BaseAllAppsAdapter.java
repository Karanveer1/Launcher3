...
public abstract class BaseAllAppsAdapter<T extends Context & ActivityContext> extends
    ...
    public static final int VIEW_TYPE_MORE_SEARCH = 1 << 6;
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_EMPTY_SEARCH:
                return new ViewHolder(mLayoutInflater.inflate(R.layout.all_apps_empty_search,
                        parent, false));
            case VIEW_TYPE_MORE_SEARCH:
                return new ViewHolder(mLayoutInflater.inflate(R.layout.all_apps_more_search,
                        parent, false));
            ...
            case VIEW_TYPE_MORE_SEARCH: {
                ...
                if (info != null) {
                    ((TextView) holder.itemView).setText(mActivityContext.getString(
                            R.string.all_apps_search_more_results));
                    holder.itemView.setOnClickListener(v -> {
                        android.util.Log.w("all_apps_search_more_results","updateSearchResultsVisibility "+info.title);
                        openApplicationMarket(info.title.toString());
                    });
                }
                break;
            }
    ...
    private void openApplicationMarket(String q) {
        try {
            String str = "market://search?q=" + q;
            //http://market.android.com/search?q=pname:<package_name>
            Intent localIntent = new Intent(Intent.ACTION_VIEW);
            localIntent.setData(Uri.parse(str));
            mActivityContext.startActivity(localIntent);
        } catch (Exception e) {
            e.printStackTrace();
//            Toast.makeText(all_apps_search_more_failed, R.string.all_apps_search_more_failed, Toast.LENGTH_SHORT).show();
        }
    }
