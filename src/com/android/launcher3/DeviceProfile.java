...
public class DeviceProfile {
    ...
    private IconLabelController ilc;
    ...
    DeviceProfile(Context context, InvariantDeviceProfile inv, Info info, WindowBounds windowBounds,
        ...
        if (lam != null) {
            ilc = lam.getIconLabelController();
        }
    ...
    private void updateHotseatSizes(int hotseatIconSizePx) {
        ...
        hotseatCellHeightPx = (int) Math.ceil(hotseatIconSizePx * ICON_OVERLAP_FACTOR) + 180;

        if (isVerticalBarLayout()) {
            hotseatBarSizePx = hotseatIconSizePx + hotseatBarSidePaddingStartPx
                    + hotseatBarSidePaddingEndPx + 180;
        } else if (isQsbInline) {
            hotseatBarSizePx = Math.max(hotseatIconSizePx, hotseatQsbVisualHeight)
                    + hotseatBarBottomSpacePx + 180;
        } else {
    ...
    private void hideWorkspaceLabelsIfNotEnoughSpace() {
        float iconTextHeight = Utilities.calculateTextHeight(iconTextSizePx) * maxIconLabelLines;
    ...
    public void autoResizeAllAppsCells() {
        ...
        allAppsCellHeightPx = allAppsIconSizePx + allAppsIconDrawablePaddingPx
                + textHeight * maxIconLabelLines + (topBottomPadding * 2);
    }
    ...
    public void updateIconSize(float scale, Resources res) {
        ...
        if (isScalableGrid) {
            ...
            if (iconDrawablePaddingPx > cellPaddingY && !isVerticalLayout
                    && !isMultiWindowMode && maxIconLabelLines == 1) {
