...
public abstract class TaskViewTouchController<T extends BaseDraggingActivity>
    ...
    private void reInitAnimationController(boolean goingUp) {
        ...
        if (goingUp) {
            ...
            if (FeatureOption.SPRD_TASK_LOCK_SUPPORT && mTaskBeingDragged.isLocked()) {
                Log.d("whn", "TaskViewTouchController goingUp mTaskBeingDragged.isLocked():2");
                mEndDisplacement = -((float) mTaskBeingDragged.getHeight() / 2);
                pa = mRecentsView.createSwipeAnimation( mTaskBeingDragged,
                        mEndDisplacement, maxDuration, false);
            } else {
                Log.d("whn", "TaskViewTouchController goingUp mTaskBeingDragged.isLocked():3");
            ...
            if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                ...
                Log.d("whn", "TaskViewTouchController goingUp mTaskBeingDragged.isLocked():4");
                ...
                Log.d("whn", "TaskViewTouchController goingUp mTaskBeingDragged.isLocked():5");
    ...
    public void onDragEnd(float velocity) {
        ...
        /* UNISOC: modify for bug 1562764 @{ */
        int verticalFactor = 1;
        if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
            if (mTaskBeingDragged != null && mTaskBeingDragged.isLocked()
                    && mCurrentAnimationIsGoingUp) {
                verticalFactor *= -orientationHandler.getTaskDragDisplacementFactor(mIsRtl);
                Log.d("whn", "TaskViewTouchController 410 verticalFactor 11:" + verticalFactor);
            } else {
                verticalFactor *= orientationHandler.getTaskDragDisplacementFactor(mIsRtl);
                Log.d("whn", "TaskViewTouchController 410 verticalFactor 12:" + verticalFactor);
            }
        }
        /* @} */
        mCurrentAnimation.setEndAction(this::clearState);
        mCurrentAnimation.startWithVelocity(mActivity, goingToEnd,
                velocity * orientationHandler.getSecondaryTranslationDirectionFactor(), //* verticalFactor,
