...
public abstract class RecentsView<ACTIVITY_TYPE extends StatefulActivity<STATE_TYPE>,
    ...
    private static Bitmap mEncryptionBg;
    ...
    public static final boolean CONTAINS_CLEAR_ALL_BUTTON = !FeatureOption.SPRD_CLEAR_ALL_ON_BOTTOM_SUPPORT;
    ...
    protected void applyLoadPlan(ArrayList<GroupTask> taskGroups) {
        ...
        //sprocomm add begin
        ArrayList<String> lockapplist = null;
        if (userManager.isSystemUser()) {
            lockapplist = getLockAppList(getContext());
        }
        //end
        ...
        for (int i = taskGroups.size() - 1; i >= 0; i--) {
            ...
            if (isRemovalNeeded && groupTask.hasMultipleTasks()) {
                ...
                //sprocomm add begin
                if (userManager.isSystemUser()) {
                    boolean appLockEnabled = hasAppLockCipher(getContext());
                    taskView.setAppLocked(isAppLocked(lockapplist, taskView.mTask != null ? taskView.mTask.key.getComponent().getPackageName() : null) && appLockEnabled);
                }
                //end
            }
    ...
    //sprocomm add begin
    public boolean hasAppLockCipher(Context context) {
        ContentResolver contentResolver = context.getContentResolver();
        boolean hasCipher = false;
        Uri uri = Uri.parse("content://com.android.settings.provider.fpdata.share/lock_app_cipher_data");
        if (contentResolver != null) {
            Cursor query = contentResolver.query(uri, null, "cipher_status =1", null, null);
        ...
        //android.util.Log.e("AppLock","hasAppLockCipher " + hasCipher);
        return hasCipher;
    }

    private ArrayList<String> getLockAppList(Context context) {
        ArrayList<String> result = new ArrayList<String>();
        Uri uri = Uri.parse("content://com.android.settings.provider.fpdata.share/lock_app_data");
        Cursor cursor = context.getContentResolver().query(uri, new String[]{"pkg_name"}, "lock_status =?", new String[]{"1"}, null);
        String str = null;
    ...
    public Bitmap getEncryptionBg() {
        return mEncryptionBg;
    }

    private void loadEncryptionBg() {
        if (mEncryptionBg == null) {
            mEncryptionBg = BitmapFactory.decodeResource(getResources(), R.drawable.applock_encryption_bg);
        }
    }

    private boolean isAppLocked(ArrayList<String> lockapplist, String apppkg) {
        if (lockapplist == null) {
            return false;
        }
        for (int j = 0; j < lockapplist.size(); j++) {
            if (apppkg.equals(lockapplist.get(j))) {
                return true;
            }
        }
        return false;
    ...
    private void removeTasksViewsAndClearAllButton() {
        for (int i = getTaskViewCount() - 1; i >= 0; i--) {
            /* UNISOC: modify for bug 1603924
            **Determine the lock animation direction by the current screen orientation and whether the home screen rotation is allowed.
            */
            TaskView taskView = requireTaskViewAt(i);
            if (taskView != null) {
                removeView(taskView);
    ...
    protected void onTaskStackUpdated() {
        ...
        updateEmptyMessage();
        mActivity.getAppMonitor().onTaskStackUpdated(getTaskViewCount() > 0);
    }
    ...
    public void setFullscreenProgress(float fullscreenProgress) {
        ...
        ClearAllController clearAllController = mActivity.getAppMonitor().getClearAllController();
        if (clearAllController != null) {
            clearAllController.setFullscreenProgress(mFullscreenProgress);
        }

        MeminfoController meminfoController = mActivity.getAppMonitor().getMeminfoController();
        if (meminfoController != null) {
            meminfoController.setFullscreenProgress(mFullscreenProgress);
        }
    ...
    private void animateRecentsRotationInPlace(int newRotation) {
        if (mOrientationState.isRecentsActivityRotationAllowed()) {
            ...
            mActivity.getAppMonitor().onOrientationStateChanged();
            return;
        }
    ...
    private void createInitialSplitSelectAnimation(PendingAnimation anim) {
        ...
        android.util.Log.d("pqq","hide xxxxx");
        ClearAllController clearAllController = mActivity.getAppMonitor().getClearAllController();
        if (clearAllController != null) {
            clearAllController.setIsSlitScreen(true);
        }

        MeminfoController meminfoController = mActivity.getAppMonitor().getMeminfoController();
        if (meminfoController != null) {
            meminfoController.setIsSlitScreen(true);
        }
    ...
    public PendingAnimation createAllTasksDismissAnimation(long duration) {
        ...
        mPendingAnimation.addEndListener(isSuccess -> {
            if (isSuccess) {
                ...
                finishRecentsAnimation(true /* toRecents */, false /* shouldPip */, () -> {
                    Log.d("whn", "Recentview FeatureOption.SPRD_TASK_LOCK_SUPPORT.get() 01");
                    if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                        Log.d("whn", "Recentview FeatureOption.SPRD_TASK_LOCK_SUPPORT.get() 02");
                        removeUnlockTaskViews();
                    } else {
                        Log.d("whn", "Recentview FeatureOption.SPRD_TASK_LOCK_SUPPORT.get() 03");
                        UI_HELPER_EXECUTOR.getHandler().post(
						ActivityManagerWrapper.getInstance()::removeAllRecentTasks);
                        removeTasksViewsAndClearAllButton();
                    }
                    MeminfoHelper.getInstance(getContext()).showReleaseMemoryToast(getContext(), count >= getTaskViewCount());
                    startHome();
                });
            }
            mPendingAnimation = null;
    ...
    private void updateEmptyStateUi(boolean sizeChanged) {
        ...
        if (mShowEmptyMessage && hasValidSize) {
            int availableWidth = mLastMeasureSize.x - mEmptyMessagePadding - mEmptyMessagePadding;
            if(mEmptyTextLayout == null){
    ...
    protected void maybeDrawEmptyMessage(Canvas canvas) {
        if (mShowEmptyMessage && mEmptyTextLayout != null) {
            ...
            if(getScrollX() > getScrollY()){
                canvas.translate(getScrollX()+ (mTempRect.left - mTempRect.right) / 2,(mTempRect.top - mTempRect.bottom) / 2);
            }else{
                canvas.translate((mTempRect.left - mTempRect.right) / 2,getScrollY()+(mTempRect.top - mTempRect.bottom) / 2);
            }
    ...
    private void removeUnlockTaskViews() {
        for (int i = 0; (getTaskViewCount() - i) > 0; ) {
            ...
            // UNISOC Add for bug 1664039, Fixed Coverity.
...
//        }
    ...
    private void addSwipeAnimations(final View taskView, AnimatorSet anim,
        ...
        int rotation = mOrientationHandler.getRotation();
        boolean canRecentsActivityRotate = mOrientationState.canRecentsActivityRotate();
        anim.setDuration(duration).setInterpolator(LINEAR);
        if (rotation == ROTATION_90 && !canRecentsActivityRotate) {
            anim.play(ObjectAnimator.ofFloat(taskView, TRANSLATION_X, -endDisplacement));
        } else if (rotation == ROTATION_270 && !canRecentsActivityRotate) {
            anim.play(ObjectAnimator.ofFloat(taskView, TRANSLATION_X, endDisplacement));
        } else {
            anim.play(ObjectAnimator.ofFloat(taskView, TRANSLATION_Y, endDisplacement));
        }
    }

    public PendingAnimation createSwipeAnimation(TaskView taskView, float endDisplacement,
                                                 long duration, boolean needLockTask) {
        if (mPendingAnimation != null) {
