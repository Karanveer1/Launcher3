...
import com.sprd.ext.lockicon.FrameLayoutWithLockIcon;
 ...
 */
public class TaskView extends FrameLayoutWithLockIcon implements Reusable {
    ...
    public void setAppLocked(boolean islock) {
        ...
        if (getRecentsView() != null) {
            this.mSnapshotView.setAppLocked(this.mIsAppLocked);
            this.mSnapshotView.setEncryptionBg(getRecentsView().getEncryptionBg());
        }
    }
    ...
    public RunnableList launchTaskAnimated() {
        if (mTask != null) {
            ...
            opts.options.setPendingIntentBackgroundActivityStartMode(
        ...
        if (visible) {
                ...
                mThumbnailLoadRequest = thumbnailCache.updateThumbnailInBackground(
                        mTask, thumbnail -> {
                            ...
                            if (mTask == null || mTask.key == null || !mTask.key.equals(originalKey)) {
                                Log.d("whn", "TaskView onTaskListVisibilityChanged TaskView final Task.TaskKey originalKey 01");
                                return;
                            }
                        });
            }
            if (needsUpdate(changes, FLAG_UPDATE_ICON)) {
                mIconLoadRequest = iconCache.updateIconInBackground(mTask,
                        (task) -> {
                            if (mTask == null || mTask.key == null || !mTask.key.equals(originalKey)) {
                                Log.d("whn", "TaskView onTaskListVisibilityChanged TaskView final Task.TaskKey originalKey 02");
                                return;
                            }
    ...
    protected void setIcon(IconView iconView, @Nullable Drawable icon) {
        if (icon != null) {
            ...
            mSnapshotView.setIcon(icon);
            mSnapshotView.invalidate();
        } else {
    ...
    protected void setThumbnailOrientation(RecentsOrientedState orientationState) {
        ...
        setOrientationState(orientationHandler, snapshotParams, isRtl, thumbnailPadding);
        Log.d("whn", "TaskView setOrientationState:");
