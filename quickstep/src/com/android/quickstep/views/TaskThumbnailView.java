...
public class TaskThumbnailView extends View {
    ...
    //sprocomm add begin
    private Bitmap mEncryptionBg;
    private String mStrEncryption;
    private Paint mEncryptionDrawPaint;
    private Paint mEncryptionDrawTextPaint;
    private Drawable mIconDrawable;
    private boolean mIsAppLocked;
    private LightingColorFilter mLightingColorFilter;
    private final Paint mAppLockPaint;

    public boolean isAppLock() {
        return this.mIsAppLocked;
    }

    public void setAppLocked(boolean islock) {
        this.mIsAppLocked = islock;
    }

    public void setEncryptionBg(Bitmap bm) {
        this.mEncryptionBg = bm;
    }
    ...
    public TaskThumbnailView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        this.mEncryptionDrawPaint = new Paint();
        this.mEncryptionDrawTextPaint = new Paint();
        Paint paint5 = new Paint(1);
        this.mAppLockPaint = paint5;
        this.mStrEncryption = getResources().getString(R.string.app_encryption);
        this.mEncryptionDrawPaint.setColorFilter(this.mLightingColorFilter);
        this.mEncryptionDrawPaint.setFilterBitmap(true);
        this.mEncryptionDrawPaint.setAntiAlias(true);
        this.mEncryptionDrawTextPaint.setColorFilter(this.mLightingColorFilter);
        this.mEncryptionDrawTextPaint.setFilterBitmap(true);
        this.mEncryptionDrawTextPaint.setAntiAlias(true);
        this.mEncryptionDrawTextPaint.setTextSize(30.0f);
        this.mEncryptionDrawTextPaint.setColor(-16119286);
        //end
    }
    ...
    public void drawOnCanvas(Canvas canvas, float x, float y, float width, float height,
        ...
        if (mIsAppLocked) {
            ...
            if (getContext().getResources().getConfiguration().uiMode == 33) {
                this.mBackgroundPaint.setColor(-16777216);
            } else {
                this.mBackgroundPaint.setColor(-1);
            }
            canvas.drawRoundRect(x, y, width, height, cornerRadius, cornerRadius, this.mBackgroundPaint);
            Bitmap bitmap = this.mEncryptionBg;
            if (bitmap != null) {
                float y3 = (float) ((viewHeight - this.mEncryptionBg.getHeight()) / 2);
                canvas.drawBitmap(this.mEncryptionBg, (float) ((viewWidth - bitmap.getWidth()) / 2), y3, this.mEncryptionDrawPaint);
                y2 = y3 + ((float) this.mEncryptionBg.getHeight());
            } else {
                y2 = y;
            }
            Drawable drawable = this.mIconDrawable;
            if (drawable != null) {
                Rect iconbound = drawable.getBounds();
                int iconwidth = iconbound.right - iconbound.left;
                int iconheight = iconbound.bottom - iconbound.top;
                canvas.save();
                canvas.translate((float) ((viewWidth - iconwidth) / 2), (float) ((viewHeight - iconheight) / 2));
                this.mIconDrawable.draw(canvas);
                canvas.restore();
            } else {
                Task task = this.mTask;
                if (!(task == null || task.icon == null)) {
                    int iconwidth2 = this.mTask.icon.getBounds().width();
                    int iconheight2 = this.mTask.icon.getBounds().height();
                    canvas.save();
                    canvas.translate((float) ((viewWidth - iconwidth2) / 2), (float) ((viewHeight - iconheight2) / 2));
                    this.mTask.icon.draw(canvas);
                    canvas.restore();
                }
            }
            if (getContext().getResources().getConfiguration().uiMode == 33) {
                this.mEncryptionDrawTextPaint.setColor(-1);
            } else {
                this.mEncryptionDrawTextPaint.setColor(-16777216);
            }
            String str = this.mStrEncryption;
            canvas.drawText(str, (((float) viewWidth) - this.mEncryptionDrawTextPaint.measureText(str)) / 2.0f, 40.0f + y2, this.mEncryptionDrawTextPaint);
        } else {
